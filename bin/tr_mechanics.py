...
def write_config_file(name):
    ...
    # print("tr_mechanics.py: ------- sub.numx, sub.numy = ", sub.numx, sub.numy)
...
def run_done_func(s, rdir):
    ...
    if nanoHUB_flag:
        ...
        os.system("submit  mail2self -s 'nanoHUB tr_mechanics' -t 'Your Run completed.'&")
...
def run_sim_func(s):
    ...
    if nanoHUB_flag:
        if remote_cb.value:
            s.run(run_name, "-v ncn-hub_M@brown -n 8 -w 1440 tr_mechanics-r7 config.xml")   # "-r7" suffix??
        else:
...
#    write_config_file(new_config_file)
    with output_widget:
        ...
        path = "/content/tr_mechanics/bin/myproj"
